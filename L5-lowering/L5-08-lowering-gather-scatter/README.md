...
4. 说出"不属于我的位置"该填什么（**提示：是归约的单位元，不一定是 0**）。
...
**为什么填 0 对 `sum` 安全**：`all_reduce` 用**加法**，而 `0` 是加法单位元。

> **★ 修正（L6-05 实测）**：填充值**不是固定的 0**，而是**归约的单位元**：
> `sum` → `0`、`min` → **`+∞`**、`max` → **`−∞`**。
> 因为 `min(0, 3) = 0` ≠ `3` —— 用 0 配 `min` 会**出错**。
> 详见 **L6-05** 与 **L5-09**（"填充值必须是单位元"）。
